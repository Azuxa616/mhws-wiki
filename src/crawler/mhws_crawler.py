...
def main():
    # 解析命令行参数
    parser = argparse.ArgumentParser(description='魔物猎人Wilds数据爬虫')
    ...
    # 创建爬虫实例
    crawler = MHWSCrawler(data_dir=args.data_dir)
    
    if args.monster:
        # 爬取特定怪物
        success = crawler.crawl_by_name(args.monster)
        if success:
            logging.info(f"成功爬取怪物: {args.monster}")
        else:
            logging.error(f"爬取怪物失败: {args.monster}")
    elif args.all:
        # 获取怪物列表
        ...
        logging.info(f"数据已保存到 {crawler.data_dir} 目录")
    else:
        # 默认只获取并保存怪物列表
        monster_list = crawler.get_monster_list()
        logging.info(f"获取到 {len(monster_list)} 个怪物信息")
